...
def predict_kill_numbers(df, target_period, window_size=30):
    ...
    for pos in positions:
        ...
        if len(X) > 5:
            ...
            for num in range(10):
                y_binary = (np.array(y) == num).astype(int)
                if sum(y_binary) > 0:
                    rf = RandomForestClassifier(n_estimators=30, random_state=42)
                    rf.fit(X_scaled, y_binary)
                    prob = rf.predict_proba(X_scaled[-1:])[0][1]
                    ml_probs[num] = prob
...
if st.button("🔮 开始预测杀号", type="primary"):
    with st.spinner("正在分析历史数据，融合多模型预测中..."):
        result = predict_kill_numbers(df, target_period, window_size)
    ...
    if result is None:
        ...
        with st.expander("📈 查看详细分析"):
            st.write("预测方法说明：")
            st.write("1. **随机森林**：基于历史特征训练的分类模型")
            st.write("2. **马尔可夫链**：分析号码状态转移概率")
            st.write("3. **遗漏分析**：统计号码遗漏值（冷号/热号）")
            st.write("4. **趋势分析**：近期走势回归分析")
            st.write("---")
            st.write("杀号逻辑：选择集成模型预测概率最低的2个号码")
