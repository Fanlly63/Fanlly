...
# 创建标签页
tab1, tab2, tab3 = st.tabs(["📊 位置级排行榜", "🎯 分拆跟单", "⚙️ 设置"])
    ...
    if st.button("🚀 生成分位置排行榜", type="primary", use_container_width=True):
        ...
        for idx, position in enumerate(positions):
            with cols[idx]:
                ...
                for rank, expert in enumerate(leaderboard[:3], 1):  # 只显示前3
                    medal = ["🥇", "🥈", "🥉"][rank-1]
                ...
                if champion['accuracy'] > 0:
                    st.success(f"🏆 {position}推荐：{champion['expert']}")
            ...
            for idx, position in enumerate(positions):
                with cols[idx]:
                    ...
                    <div style="background:#e3f2fd;padding:15px;border-radius:10px;text-align:center;">
                        ...
                        <div style="font-size:24px;margin:10px 0;">{expert_name.split()[0]}</div>
...
with tab3:
    ...
    **推荐模式：**
    - **全自动分配**：省心，系统为每个位置选胜率最高的
    - **手动分配**：如果你发现万位最近"趋势明显"，手动指定趋势专家
    """)
